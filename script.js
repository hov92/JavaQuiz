...
var answers_list = document.querySelector(".answers_list");
...
function showQuetions(index){
    ...
    answers_list.innerHTML = option_tag; //adding new div tag inside option_tag
    
    var option = answers_list.querySelectorAll(".option");
...
function optionSelected(answer){
    ...
    var allOptions = answers_list.children.length; //getting all option items
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        ...
        for(i=0; i < allOptions; i++){
            if(answers_list.children[i].textContent == correcAns){ //if there is an option which is matched to an array answer 
                answers_list.children[i].setAttribute("class", "option correct"); //adding green color to matched option
                answers_list.children[i].insertAdjacentHTML("beforeend", tickIconTag); //adding tick icon to matched option
                console.log("Auto selected correct answer.");
            }
        }
    }
    for(i=0; i < allOptions; i++){
        answers_list.children[i].classList.add("disabled"); //once user select an option then disabled all options
    }
